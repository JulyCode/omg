...
void readtopo()
{
  ...
  // Read information on priority areas with high resolution 
  fscanf(fp,"%d\n",&nprio);
  priodat = (float**) calloc(nprio,sizeof(float*));
  for (i=0; i<nprio; i++) priodat[i] = (float*) calloc(5,sizeof(float));

  for (i=0; i<nprio; i++) fscanf(fp,"%10s %f %f %f %f %f\n", 
				    str, &priodat[i][0],&priodat[i][1],&priodat[i][2],&priodat[i][3],&priodat[i][4]);
  fclose(fp);

  cfl_glob=dx_4000/200.;
  mindepth=0.1*(dx_min*dx_min)/(cfl_glob*cfl_glob);

  printf("global CFL number: %6.2f sec\n",cfl_glob);
  printf("minimal depth: %6.2f m\n",mindepth);

  printf("nmb of priority regions: %d\n",nprio);
  printf("Details of the first:\n");
  printf("%f %f %f %f %f\n", priodat[0][0],priodat[0][1],priodat[0][2],priodat[0][3],priodat[0][4]);

  // read main topography file

  fp=fopen(WORKING_DIRECTORY "topo/lon_medsea.bin32","r");
  count=fread(lon,sizeof(float),LenLon,fp);
  //printf("%d %lf\n",count,lon[0]);
  fclose(fp); 

  fp=fopen(WORKING_DIRECTORY "topo/lat_medsea.bin32","r"); 
  count=fread(lat,sizeof(float),LenLat,fp); 
  //printf("%d %lf\n",count,lat[0]); 
  fclose(fp); 

  fp=fopen(WORKING_DIRECTORY "topo/topog_medsea.bin32","r"); 
  count=fread(topo,sizeof(float),LenLon*LenLat,fp); 
  //printf("%d %f %f %f\n",count,topo[0][0],topo[1][0],topo[0][1]); 
  fclose(fp); 

  fp=fopen(WORKING_DIRECTORY "topo/topog_grad_medsea.bin32","r"); 
  count=fread(topo_grad,sizeof(float),LenLon*LenLat,fp); 
  //printf("%d\n",count); 
  fclose(fp);
     ...
     double area;
{
  ...
  if (fabs(dy-0.008333333)<0.001) dy=1./120.;
  if (fabs(dy-0.016666666)<0.001) dy=1./60.;
  ...
  // Depth in the second node COARSE AREA
  
  ix=floor((tridest[0]-lon[0])/dx);
...

{
  ...
  while (vertexloop != (vertex) NULL) {
    if (!b->jettison || (vertextype(vertexloop) != UNDEADVERTEX)) {
      ...
      fprintf(outfile, "%4d    %.17f  %.17f", vertexnumber, vertexloop[0],
              vertexloop[1]);
      for (i = 0; i < m->nextras; i++) {
        /* Write an attribute. */
...

{
  ...
  char* arguments[4];
  arguments[0] = "";
  arguments[1] = SWITCHES;
  arguments[2] = INPUT_FILE;
  arguments[3] = NULL;
  argv = arguments;
